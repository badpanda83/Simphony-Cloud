...
import { EmailApprovalProvider, GuardedResetExecutionPipeline, SlackApprovalProvider, } from "../reset-automation/providers.js";
...
passwordResetApprovalRouter.post("/workflows/password-reset/inbound", asyncHandler(async (req, res) => {
    requireFeatureEnabled();
    const { externalMessageId, sender, subject, resetLink, receivedAt, source, } = req.body ?? {};
...
passwordResetApprovalRouter.get("/workflows/password-reset/approve", asyncHandler(async (req, res) => {
    requireFeatureEnabled();
...
passwordResetApprovalRouter.get("/workflows/password-reset/deny", asyncHandler(async (req, res) => {
    requireFeatureEnabled();
    const token = String(req.query.token ?? "");
    if (!token) {
        res.status(400).json({ error: "token query parameter is required" });
...
passwordResetApprovalRouter.get("/workflows/password-reset/status/:requestId", asyncHandler(async (req, res) => {
    requireFeatureEnabled();
    const request = orchestrator.getStatus(String(req.params.requestId));
    res.json({ ok: true, request });
}));
//# sourceMappingURL=password-reset-approval.js.map