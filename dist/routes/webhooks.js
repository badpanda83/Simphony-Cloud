...
webhooksRouter.post("/webhooks/simphony", asyncHandler(async (req, res) => {
    const rawBody = JSON.stringify(req.body);
    const signature = req.header("x-signature") ??
        req.header("X-Signature") ??
        req.header("simphony-signature");
    if (config.webhook.hmacKey) {
        const valid = verifySimphonyHmac(rawBody, signature, config.webhook.hmacKey);
        if (!valid) {
    ...
    console.info("[webhook]", payload.messages?.map((m) => m.messageType?.id).join(", ") ?? "empty");
    res.status(200).json({ received: true, count: payload.messages?.length ?? 0 });
}));
/** Manual test — forwards sample payload to webhook.site */
webhooksRouter.post("/webhooks/test-forward", asyncHandler(async (req, res) => {
    const body = req.body ?? {
        messages: [
            {
                id: "test",
