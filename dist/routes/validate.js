...
import { simphonyRequestWithSession, } from "../simphony/client.js";
...
function checkOpenTimeWithinDays(openTime, days) {
    if (!openTime)
        return false;
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    return new Date(openTime).getTime() >= cutoff;
}
validateRouter.post("/workflows/setup/validate", asyncHandler(async (req, res) => {
    const body = req.body;
    const required = [
        "simphonyHost",
        "clientId",
    ...
    const request = (opts) => simphonyRequestWithSession(session, {
        ...opts,
    ...
    try {
        ...
        if (!menuId) {
            ...
            const categories = asArray(data.familyGroups, [
                "familyGroups",
            ]);
            steps.push({
    ...
    try {
        ...
        const recentChecks = allChecks.filter((c) => checkOpenTimeWithinDays(c.header?.openTime, 7));
