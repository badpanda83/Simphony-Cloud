...
checksRouter.get(
  "/api/v1/checks/:checkRef/printed",
  asyncHandler(async (req, res) => {
    const ctx = contextFromRequest(req);
    const { data, status } = await simphonyRequest({
      method: "GET",
 ...
 */
checksRouter.post(
  "/workflows/checks/:checkRef/items",
  asyncHandler(async (req, res) => {
    const ctx = contextFromRequest(req);
